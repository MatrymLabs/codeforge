...
result: >
  ...
  5 of 18, and engine-falsifiability stayed at 7 of 18.
...
pattern_screen:
  ...
  verdict_note: COMPLETE; all four movement probes now read Blueprint-defined rooms and exits, while the 7 of 18 engine-falsifiability floor holds
```

IN PLAIN TERMS: The movement checks now use the rooms and exits from whichever Blueprint is being tested, so swapping Blueprints changes five of the eighteen answers instead of only the room-coverage answer. The engine comparison stayed at seven falsifiable checks, so the new world sensitivity did not buy its number by weakening the engine test. The key concept is data-driven probing: the test setup must come from the data it claims to measure.
