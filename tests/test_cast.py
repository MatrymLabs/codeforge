...
def test_planning_blocks_when_the_starter_pack_is_missing(tmp_path: Path) -> None:
    ...
    (tmp_path / "content" / "blueprints").mkdir(parents=True)  # no 'not-installed' pack inside
...
def test_generate_pours_the_engine_seed_and_scaffold(tmp_path: Path) -> None:
    ...
    assert (out / "content" / "blueprints" / "first-forge" / "rooms.yaml").is_file()
    for scaffold in (
        "cast_manifest.json",
        "README.md",
        "seed.toml",
...
def test_generate_carries_only_its_own_seed_pack(tmp_path: Path) -> None:
    ...
    out = generate_cast(plan, tmp_path / "out", root=tmp_path)
    assert (out / "content" / "blueprints" / "first-forge").is_dir()
    assert not (
        out / "content" / "blueprints" / "other-pack"
