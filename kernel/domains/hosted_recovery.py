...
spawn). This proves the thing the server actually hosts: the installed seed dir, manifest and all:
  ...
  * `verify_seed_recovery(seed_name, seed_root, snap)` -- the RESTORE + VERIFY: re-read every
  ...
  * `prove_hosted_recovery(spec, seed_root, *, seed_name=, title=)` -- the whole loop: INSTALL
...
def verify_seed_recovery(
    seed_name: str, seed_root: Path, snap: dict[str, str]
...
def prove_hosted_recovery(
    ...
    *,
    ...
    world = install_world(spec, seed_root, seed_name=seed_name, title=title)
    ...
    return verify_seed_recovery(world.seed_name, seed_root, snap)
