...
class Item(TypedDict):
    ...
    # runtime items are instances cloned from it (kernel.world.items.clone). Seed items are their
    ...
    # home room when its area comes due (kernel.world.zones), a fresh instance spawns there. Default
    ...
    # the pickup is not always in the same spot (kernel.world.zones._spawn_wanderers). Requires
...
class Npc(TypedDict):
    ...
    # A RAID boss: a boss-tier foe on a WEEKLY lockout (kernel.world.lockouts), meant for a party,
    # not a solo lap. It composes the trinity + shared rooms + threat + the once-a-week reward.
    raid: NotRequired[bool]
    # WANDER: an ambient peaceful NPC that drifts between rooms on the beat (kernel.world.roaming),
    ...
    # Optional (boss-tier): a TELEGRAPHED SPECIAL (kernel.world.boss_specials). {telegraph, mult?,
    ...
    # AMBIENT wildlife: a mass-generated wilderness creature (kernel.world.bestiary), not a notable
...
class Ability(TypedDict):
    ...
    # "weaken" (soften blows) | "taunt" (force a foe's aggro) | "cleanse" (purge afflictions) |
    # "buff" (empower an ally's blows for a spell)
    kind: str
    ...
    # this many beats; a landed strike advances the combat clock and thaws it (kernel.world.combat_
...
class Profession(TypedDict):
    """A maker's TRADE: a data-driven skill track leveled by DOING (kernel.world.professions).
...
class GearSet(TypedDict):
    ...
    data: sets are YAML, validated at load; the bonus math lives in kernel.world.gearsets.
    """
...
class Zone(TypedDict):
    ...
    declared here as data; the beat-driven scheduler that reads it lives in `kernel/world/zones.py`.
