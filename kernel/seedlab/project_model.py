...
class Provenance:
    ...
    def __post_init__(self) -> None:
        if not self.source_id or not self.source_id.strip():
            raise SeedLabError("provenance needs a non-empty source_id")
...
def _string_list(spec: dict, key: str) -> list[str]:
    ...
    if not isinstance(raw, list):
        raise SeedLabError(f"'{key}' must be a list, got {type(raw).__name__}")
    out: list[str] = []
    for item in raw:
        if not isinstance(item, str) or not item.strip():
            raise SeedLabError(f"'{key}' entries must be non-empty strings, got {item!r}")
...
def _relationships(spec: dict) -> list[Relationship]:
    ...
    if not isinstance(raw, list):
        raise SeedLabError(f"'relationships' must be a list, got {type(raw).__name__}")
    ...
    for triple in raw:
        ...
        if not all(isinstance(x, str) and x.strip() for x in (subject, verb, obj)):
            raise SeedLabError(f"relationship parts must be non-empty strings: {triple!r}")
...
def extract_model(source: ProjectSource) -> ProjectModel:
    ...
    Fails loud (SeedLabError) on a bad spec -- a model is only as trustworthy as its source."""
    spec = source.spec()
    if not isinstance(spec, dict):
        raise SeedLabError(f"a project spec must be a mapping, got {type(spec).__name__}")
    identity = spec.get("identity")
    if not isinstance(identity, str) or not identity.strip():
        raise SeedLabError("a project spec needs a non-empty 'identity'")
    return ProjectModel(
