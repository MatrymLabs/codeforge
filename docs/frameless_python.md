# Architecture-First Python (CodeForge's identity)

*CodeForge is **architecture-first Python**: Python's standard library and my own architecture
are the spine; external tools are professional integrations that earn their place, not the
identity of the project. ("Frameless" was the original framing, and this file keeps that name;
the working doctrine and its adoption gate are in [`full_stack_forge_decision.md`](full_stack_forge_decision.md).)*

## What "architecture-first" means here
...
What CodeForge is **not defined by** any one tool:

- not "a Ruff project," "a pytest project," or "a FastAPI app" - FastAPI is one of several
  surfaces onto the engine, not its identity.
- not "an Evennia clone" or "just a wrapper around tools."

## The evidence the architecture is the spine
...
## The architecture-first rule (ask before adding a tool)
...
An interviewer who sees "architecture-first Python" should read: *this person built the architecture,
