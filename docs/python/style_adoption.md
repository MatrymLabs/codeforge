...
| §2 kernel/adapters/content + import-linter | **APPROVED, in migration** | Owner approved the literal restructure (2026-08-02). A multi-slice campaign: move the 112 `parts/` modules into `kernel/` (pure) + `adapters/` (IO/framework) + `content/` (seeds/data), rewiring every import, with import-linter enforcing the one-way direction. Staged, not big-bang; see the migration plan. |
