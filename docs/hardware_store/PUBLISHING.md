...
- Version is `0.3.0`; bump it in `shelf_pour._pyproject` before each release.
...
- The 2 engine-coupled twins (`console`, `observability`) keep their integration tests in this repo;
  the poured package ships every remaining engine-free twin (34 today, over a 36-core shelf) and
  passes them with no engine present. The pour derives both counts from the live shelf, so they
  track it rather than a hand-maintained number (`test_shelf_pour.py` pins the parity).
