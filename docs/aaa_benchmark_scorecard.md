...
| **Combat systems** | ~40% | ~55% | 127 abilities across **9 ability kinds** (strike/heal/brand/daze/weaken/taunt/cleanse/buff + a lifesteal `drain`), 10 damage types, boss phases + telegraphed specials + afflictions. Party combat shares XP + round-robin loot; the trinity seams now exist (per-NPC threat/aggro table + taunt, ally-targeted heals). A raid bounty scales with the co-located cohort (#711). A **kit-density pass** brought every calling to a coherent kit (17 iconic callings at ~5 abilities; no calling left at kill-only depth) that wields the deep kinds (DoT/control/lifesteal). Gaps: raid *difficulty*-scaling (reward-scaling done), deeper boss-mechanic variety (3 kinds: strike, mend, vampiric drain). |
