...
| **Project source → structured model** | **PROTOTYPED** | `kernel/seedlab/project_model.py` |
| **Seed identity + lifecycle (as an addressable entity)** | **PROTOTYPED** | `kernel/seedlab/kernel.py` (MOD-10.052) — create/start/stop/archive a Seed with owner authz, an audit trail, and file-backed persistence that survives restart; distinct from the `FORGE_SEED` game pack |
| **Functional Project Hub (enter + inspect a Seed)** | **PROTOTYPED** | `kernel/seedlab/project_hub.py` (MOD-10.053) — a Seed location that renders identity/status/facets as a text `look` + its sub-verbs AND a versioned structured client contract, both from one source of truth; empty facets read "none yet (Stage N)" |
| **Local source connector (read-only, path-bounded)** | **PROTOTYPED** | `kernel/seedlab/source_connector.py` (MOD-10.054) — register a local dir with provenance; list/search/read only approved files; refuses traversal, absolute paths, symlink escapes, and a secret denylist; feeds the Hub's `sources` facet |
| **Source → project model (persisted, honest)** | **PROTOTYPED** | `kernel/seedlab/source_modeler.py` (MOD-10.055) + `model_store.py` (MOD-10.056) — extract a `ProjectModel` from a registered source (identity/entities/interfaces/provenance derived; everything else marked in `unknowns`), persist it (survives restart), and light up the Hub's `models` facet linked to source evidence |
| **Controlled build/test execution** | **PROTOTYPED** | `kernel/seedlab/tool_runner.py` (MOD-10.057) — run an allowlisted, shell-free command inside an approved source with cwd boundary + timeout + output cap + secret redaction; refuse an unlisted profile; persist each run (`FileRunLog`, survives restart) into the Hub's `builds`/`tests` facets |
| **First generated target (a runnable CLI)** | **PROTOTYPED** | `kernel/seedlab/cli_generator.py` (MOD-10.058) — generate a small, runnable Python CLI from a validated model (reproducible, sha256-checksummed, provenance-carrying); `validate_runs`/`validate_tests` prove it RUNS and its generated tests PASS via the Stage-5 runner. **Closes the First Platform Proof: source → model → generate → run → tests pass → artifact.** |
| **Second generated target (a runnable web API)** | **PROTOTYPED** | `kernel/seedlab/web_api_generator.py` (MOD-10.073) — generate a small, runnable **stdlib-only WSGI web service** from the same validated model (routes `/`, `/health`, one `/<action>` per model action; JSON responses; `serve()` on `wsgiref`). Reproducible, sha256-checksummed, provenance-carrying; a `--check` mode lets `validate_runs` prove it RUNS without binding a port, and `validate_tests` proves its generated pytest suite PASSES — both via the Stage-5 runner. **Proves the generation pipeline GENERALIZES beyond one target family** (the "no universal generation until tested generators exist" bar, now cleared twice). Reuses `cli_generator`'s artifact record + helpers (no duplication). |
...
| **In-MUD `workspace` verb (THE SEED IS THE MUD, in-world)** | **PROTOTYPED** | `kernel/seedlab/workspace_verb.py` (MOD-10.060) + `forge.py` (CMD-10.032, CORE/owner) — an owner lists/creates/inspects/starts/stops engineering Seeds and lists their models from inside the running MUD; the text half of the workspace surface (GMCP half: `workspace_gmcp.py`, #727) |
| **Aethryn as a reference Seed (the game is one *kind* of Seed)** | **PROTOTYPED** | `kernel/seedlab/reference_seed.py` (MOD-10.061) — idempotently registers the flagship game as a Seed in the same Kernel (game-agnostic; adds nothing game-specific), so `workspace list` shows Aethryn `[reference game]` beside engineering Seeds. First brick of Stage 8 (full re-home is a later program) |
...
`kernel/seedlab/project_model.py` (MOD-10.051) proves the directive's step 9 — *the Seed consumes
...
It is **isolated in `kernel/seedlab/`, not `kernel/world/`** — the game Seed model is untouched.
...
no restart recovery. `kernel/seedlab/kernel.py` (MOD-10.052) builds the first brick:
...
- A CLI (`python3 -m kernel.seedlab.kernel create|list|status|start|stop|archive`) makes the
  lifecycle real and inspectable from the shell.

Isolated in `kernel/seedlab/`: no `kernel/world/` import, no `FORGE_SEED`, no game coupling. **Honest
...
`kernel/seedlab/project_hub.py` (MOD-10.053) gives a Seed a place you can ENTER and inspect. It
...
`kernel/seedlab/source_connector.py` (MOD-10.054) is the first real project-source connector and the
...
`kernel/seedlab/source_modeler.py` (MOD-10.055) turns a registered `LocalSource` into a `ProjectModel`
...
`unknowns`, and `to_dict`/`from_dict`. `kernel/seedlab/model_store.py` (MOD-10.056) persists models
...
`kernel/seedlab/tool_runner.py` (MOD-10.057) is the controlled-execution primitive. `run_tool` runs an
...
`kernel/seedlab/cli_generator.py` (MOD-10.058) is the recenter's payoff. `generate_cli` turns a
...
pass** (CLI Generator). All isolated in `kernel/seedlab/`, the game untouched, every claim labeled.
...
`kernel/seedlab/workspace_verb.py` (MOD-10.060), wired into `forge._build_commands` as **CMD-10.032**
...
GMCP client half is `kernel/seedlab/workspace_gmcp.py` (#727), reading the same seedlab modules so the
...
`kernel/seedlab/reference_seed.py` (MOD-10.061) proves the recenter's core claim concretely: the
...
2. Promote the strongest cards from `prototype` → `beta` once a second consumer proves each outside
   `kernel/seedlab/` (not yet ripe — each card has one consumer today; promoting now would overclaim).
...
now works inside the running MUD. An optional **allowed-sources boundary** (`$SEEDLAB_SOURCES`) confines
`connect` to a configured base when set (defence in depth on top of owner-gating); unset keeps the
owner-trusted default.
