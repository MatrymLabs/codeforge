...
> schema in `kernel/world/seed.py`, real-time combat ticks, per-tick resource regen, and independent
...
`ModifierStack` (see `kernel/world/equipment.py`), so gear tunes a build without the job knowing about
any specific item.
...
Two independent tracks, already implemented (`kernel/world/progression.py`):
...
**A job is a data record.** It maps onto the shipped `Job` schema (`kernel/world/seed.py`) exactly:
