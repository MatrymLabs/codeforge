...
- **Build:** proactive/aggressive NPCs (`kernel/world/aggression.py` + a shared blow resolution in
  `kernel/world/combat.py` + a typed `StrikeFrame`), wired into the engine tick.
...
## AI contribution
AI-assisted implementation of `kernel/world/aggression.py` (`menace`), the `_resolve_npc_blow` extraction +
`open_strike` in `kernel/world/combat.py` (behavior-preserving refactor of `_counter_attack`), the
`StrikeFrame` in `kernel/world/frames.py`, the `aggressive` seed field + loud validation in `kernel/world/seed.py`,
...
## Tests / evidence
- `kernel/world/aggression.py` twinned by `tests/test_aggression.py` (acceptance: opens on the beat, reaches
...
1. **Bounded (safety).** An aggression **leash** (`kernel/world/aggression.py`, `LEASH = 5`): a foe breaks off
...
2. **Fair (human factors).** The room render telegraphs hostility (`kernel/world/npcs.py` `_presence_line`):
