...
| Sparse event-driven control plane | Wake only the subsystems an event touches; bounded queues; interrupt lane for security/policy | **Extrapolation** | High / Medium - Build now | `kernel/world/events.py` | Srivatsav et al. (2023); Su et al. (2024); Frank et al. (2025) |
...
| Bounded evaluator swarm | Specialized read-only evaluators that score + explain; **no merge authority, no self-approval** | **Extrapolation** | Medium-high / Medium - Build soon | QualityGate, EvidenceGate, AI seam | SOEN-101 (2024) |
