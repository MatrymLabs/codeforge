...
clients (Mudlet / telnet / nc / terminal)
   ...
   -> events bus (sinks per player; room broadcasts; shutdown hook)
   -> seeds/*.yaml + splash.txt (the world as data, gated at load)
   -> characters.json / accounts.json (minimal canonical state)
```
...
1. **State is canonical; text is a projection.** `render_scene`, `score_text`, and
...
3. **Derive, don't store.** A character record is job + level + xp + location + rank
