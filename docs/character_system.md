...
Both curves are locked design (`kernel/world/progression.py`); XP, JP, and TP are all awarded on a kill.
...
base = derived_stats(attributes, level)        # kernel/world/derived.py (prototype formulas)
  -> apply_equipment(...)                       # equipped gear's mods       (kernel/world/equipment.py)
  -> apply_stat_modifiers(..., perk_modifiers)  # unlocked milestone perks   (kernel/world/character_view.py)
```

Because everything flows through one stack, gear and perks stack the same way, and rebalancing
means editing one file (`kernel/world/derived.py` for the base formulas, the seed data for gear/perks).
...
- `kernel/world/score_sheet.py` - a pure `CharacterSheet` view model + `render_score_sheet(sheet, mode)`.
...
- `kernel/world/character_view.py` - `sheet_from_session(session)` does the engine-coupled assembly:
...
| Knob | Where |
|---|---|
| Base derived-stat formulas | `kernel/world/derived.py` |
| XP -> PLvl and JP -> Job Level curves (locked) | `kernel/world/progression.py` |
| TP per milestone (500) | `character_view.TP_MILESTONE` |
| Engineer costs/cooldowns/thresholds | constants at the top of `kernel/world/engineer.py` |
