...
def _blueprint(
    ...
    """BLUEPRINT: assemble a plan from the found part + manifest and validate it (never written)."""
    plan = {
...
def _diagnose(part: hardware.Part, source_text: str) -> StationReport:
    """DIAGNOSE: peak cyclomatic complexity + a structural-clone scan of the part's own source."""
    functions = complexity.scan_source(source_text)
    clones = clone_scan.find_clones({part.source: source_text})
    if not functions:
