...
`parts/bench.py` measures the engine tick single-threaded: the best case, one caller. A load test
...
def _percentile(sorted_us: list[float], fraction: float) -> float:
    """The value at `fraction` of a sorted latency list (nearest-rank, matching parts/bench.py)."""
    if not sorted_us:
        return 0.0
