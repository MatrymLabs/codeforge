...
def parse_etag(raw: str) -> ETag:
    ...
    if not isinstance(raw, str):
        raise PreconditionError(f"etag must be a string, got {type(raw).__name__}")
    m = _ETAG.match(raw.strip())
    if not m:
        raise PreconditionError(f"malformed etag: {raw!r} (expected a quoted value)")
...
def etag_for_version(version: int) -> ETag:
    ...
    if isinstance(version, bool) or not isinstance(version, int):
        raise PreconditionError("version must be a non-bool int")
    if version < 0:
        raise PreconditionError("version must be >= 0")
...
def etag_for_payload(payload: bytes, *, weak: bool = False) -> ETag:
    ...
    if not isinstance(payload, (bytes, bytearray)):
        raise PreconditionError("payload must be bytes")
...
def _parse_tag_list(header: str) -> tuple[bool, list[ETag]]:
    ...
    if not isinstance(header, str):
        raise PreconditionError("conditional header must be a string")
    trimmed = header.strip()
    if trimmed == "":
        raise PreconditionError("conditional header must not be empty")
    if trimmed == _STAR:
        return True, []
...
def if_match(current: ETag | None, header: str) -> None:
    ...
    if is_star:
        ...
        return
    if current is None:
        raise PreconditionFailed("If-Match given but resource does not exist")
    if any(current.strong_equals(t) for t in tags):
        return
    raise PreconditionFailed("If-Match did not match the current ETag")
...
def if_none_match(current: ETag | None, header: str) -> None:
    ...
    if is_star:
        if current is not None:
            raise PreconditionFailed("If-None-Match: * but resource already exists")
        return
    if current is None:
        return
    if any(current.weak_equals(t) for t in tags):
        raise PreconditionFailed("If-None-Match matched the current ETag")
...
def require_version(expected: int, actual: int) -> None:
    ...
    for label, v in (("expected", expected), ("actual", actual)):
        if isinstance(v, bool) or not isinstance(v, int):
            raise PreconditionError(f"{label} version must be a non-bool int")
    if expected != actual:
        raise StaleWrite(f"stale write: expected version {expected}, current is {actual}")


def next_version(current: int) -> int:
    """The version to store after a successful guarded write."""
    if isinstance(current, bool) or not isinstance(current, int):
        raise PreconditionError("current version must be a non-bool int")
    if current < 0:
        raise PreconditionError("current version must be >= 0")
    return current + 1
